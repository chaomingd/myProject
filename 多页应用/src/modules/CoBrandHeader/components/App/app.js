...
export default {
    ...
    methods: {
        initApp() {
			...
			if (this.$refs){
				...
				this.$refs.Computer.initCjs(cjsComputerId, aniObjComputer => {
					...
					this.cjsComputerAnim.pause(0);
					setTimeout(() => {
						this.cjsComputerAnim.gotoEnd(); // goto LastFrame
						this.startTextAnimate();
					}, 500);
        ...
        startTextAnimate() {
			const self = this;
			stagger([$('.CoBrandHeader .text-box1'), $('.CoBrandHeader .text-box2'), $('.CoBrandHeader .chip-box')],.1)
			stagger([$('.CoBrandHeader .animate'), $('.CoBrandHeader .text-box3')],.1,() => {
				setTimeout(() => {
					stagger([$('.CoBrandHeader .logo-retailer')],.5)
				},800)
			})
        },
        messageHandler(e){
